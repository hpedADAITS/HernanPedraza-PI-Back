...
const USERS = [
  { email: "admin@syncrekuest.com", displayName: "Admin Principal", role: "ADMIN" },
...
const EVENTS = [
  {
    ...
    state: "LIVE",
    startsAt: new Date(),
    settings: { allowRequests: true, requireApproval: false, votingEnabled: true, allowDownvotes: true, maxRequestsPerParticipant: 5 },
  },
  {
    name: "Electro Fest",
    ...
    settings: { allowRequests: true, requireApproval: true, votingEnabled: true, allowDownvotes: false, maxRequestsPerParticipant: 3 },
  },
  {
    name: "Rock Clásico",
...
const PARTICIPANT_NAMES = [
  "ChicoFiesta", "BailaConmigo", "RitmoLoco", "NoctámbulaMX",
  "ElMelómano", "SalsaQueen", "BeatMaster", "VibraAlta",
  "LunaRoja", "TucanBailable", "MixMaestro", "EcoNocturno",
];
...
const SONG_STATUSES = ["PENDING", "APPROVED", "PLAYING", "PLAYED", "SKIPPED", "REJECTED"];
...
async function populate() {
  ...
  for (const event of events) {
    ...
    for (const [idx, songData] of songPool.entries()) {
      ...
      const status = event.state === "ENDED" ? pick(["PLAYED", "SKIPPED"]) : pick(SONG_STATUSES);
      ...
      const song = await SongModel.create({
        ...
        startedPlayingAt: status === "PLAYING" || status === "PLAYED" ? new Date() : undefined,
      });
  ...
  const logTypes = ["EVENT_START", "SONG_APPROVE", "SONG_REJECT", "PARTICIPANT_KICK"];
  let logCount = 0;
  for (const event of events) {
